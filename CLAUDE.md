...
# Industry stats
python manage.py industry_stats                    # all campaigns
python manage.py industry_stats --campaign "Name"  # single campaign
```
...
- **Task queue**: `Task` model (persistent). Three types: `connect`, `check_pending`, `check_messages`. (``follow_up`` was merged into ``check_messages`` — the unified handler does both proactive nudges and lead replies.) Handlers in `linkedin/tasks/`, signature: `handle_*(task, session, qualifiers)`. Task rows are **lazy**: `payload = {"campaign_id": <id>}` only; the handler resolves the concrete target at execution time via a single eligibility query. Slot creation is centralized in `linkedin/tasks/scheduler.py` — no other module inserts Task rows. The planner exposes one function per type (`plan_connect_window`, `plan_follow_up_window`, `plan_check_pending_window`, `plan_check_messages_window`); each emits `1 immediate + (n-1) Poisson-spaced` slots over the next 24h working window. `working_seconds_in_window(start, end)` and `poisson_slot_times(now, n, horizon_hours)` are the spacing primitives (uniform order-statistic implementation, exactly `n` slots, mean spacing `T/(n+1)` in working time). State-transition hook `on_deal_state_entered(deal)` only stamps `Deal.next_check_pending_at` on PENDING (no Task row); all other transitions are no-ops. `reconcile(session)` runs on daemon idle: it recovers stale RUNNING tasks, then iterates campaigns × planners and re-plans any type whose pending queue is empty. Active hours are a single contiguous daily window (`ENABLE_ACTIVE_HOURS`, `ACTIVE_START_HOUR`, `ACTIVE_END_HOUR`, `ACTIVE_TIMEZONE`) — no weekend skip. `CHECK_PENDING_DAILY_CAP` (default 100) bounds the per-cycle check_pending slot count. On 401 (`AuthenticationError`), the daemon calls `session.reauthenticate()` and marks the task FAILED; the planner picks the type up on the next idle cycle. `check_messages` (unified — merged the old `follow_up` type) plans `CHECK_MESSAGES_SLOTS_PER_DAY` (default 8) slots per campaign per day. Its handler calls the Voyager conversations API to scan the inbox, filters conversations by matched `Lead.urn`, and handles both paths: (a) **lead replied** — syncs and replies via the follow-up agent, resets `unanswered_follow_up_count`; (b) **nudge due** — sends a proactive follow-up via `_handle_follow_up_nudge` when `deal.next_follow_up_at` has expired. A fallback orphan scan catches CONNECTED deals not yet in the inbox (new connections) and triggers first outreach. `FOLLOW_UP` task type is deprecated; lingering tasks are routed to `handle_check_messages`. The old `plan_follow_up_window`, `enqueue_follow_up`, and `handle_follow_up` are gone. See `BACKUP_follow_up.py` at project root for the original implementation.
- **ML pipeline**: GPR (sklearn) + BALD active learning + LLM qualification. Per-campaign models stored in `Campaign.model_blob` (DB).
- **Config**: `SiteConfig` DB singleton (LLM_PROVIDER, LLM_API_KEY, AI_MODEL, LLM_API_BASE — editable via Django Admin; `llm_provider` chooses between OpenAI/Anthropic/Google/Groq/Mistral/Cohere/openai_compatible, `llm_api_base` only consulted when provider is `openai_compatible`), `conf.py:CAMPAIGN_CONFIG` (timing/ML defaults), `conf.py` browser constants (`BROWSER_*`, `HUMAN_TYPE_*`), `conf.py` schedule constants (`ENABLE_ACTIVE_HOURS` flag, active hours/timezone/rest days), `conf.py` onboarding defaults (`DEFAULT_*_LIMIT`), `conf.py:FASTEMBED_CACHE_DIR` (persistent model cache, defaults to `<project>/.cache/fastembed/`), Campaign/LinkedInProfile models (Django Admin). `VOYAGER_REQUEST_TIMEOUT_MS` lives in `api/client.py` (constructor default on `PlaywrightLinkedinAPI`). `conf.py:DUMP_PAGES` (default `False`) — enable to save page HTML snapshots for fixture collection. **Telegram notifications**: `conf.py` contains `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` (pre-configured). `linkedin/notification.py` provides `notify_failure(FailureEvent(...))` and `send_telegram(text)`. Wired into `daemon.py` (task errors, auth failures, LLM API halts) and task handlers (connection limits, inaccessible profiles, send failures, disqualifications). Best-effort sent via stdlib `urllib` — no extra dependency.
- **Lazy accessors**: `Lead.get_profile(session)` is a pure live Voyager scrape (no DB caching of the raw dict); `Lead.get_urn(session)` reads the `urn` column and falls back to a scrape; `Lead.get_embedding(session)` lazily scrapes + embeds on first access, then caches the 384-dim bytes on the row. `Lead.embed_from_profile(profile)` reuses an in-hand profile dict to skip the scrape (used by `create_enriched_lead`). `Lead.to_profile_dict()` returns a minimal `{lead_id, public_identifier, url, meta}` dict (no `profile` key). `Lead.industry` is captured from the Voyager profile at enrichment time (`create_enriched_lead`) and lazily backfilled on any re-scrape via `get_profile()` or `embed_from_profile()`. `AccountSession.campaigns` (cached_property, list). `AccountSession.self_profile` (cached_property, re-discovers via Voyager on first access per session — no DB cache).
...
- **Django apps**: `linkedin` (main — Campaign with users M2M), `crm` (Lead with industry/embedding/Deal), `chat` (ChatMessage).
