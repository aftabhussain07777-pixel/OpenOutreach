...
5. **Session** — `get_or_create_session(profile)`, sets default campaign (first campaign).
...
3. **LLM config** — `LLM_PROVIDER`, `LLM_API_KEY`, `AI_MODEL`, `CONVERSATION_AI_MODEL`, `LLM_API_BASE` → writes to `SiteConfig` singleton in DB.
...
1. **Per-type planners** — `plan_connect_window`, `plan_check_pending_window`, `plan_check_messages_window`. (``plan_follow_up_window`` was removed when follow-up merged into ``check_messages``.) Each, when no PENDING task of its type exists for a campaign, computes the right slot count `n` for the next 24h and inserts `1 immediate + (n-1) Poisson-spaced` lazy rows. The leading immediate slot kills the cold-start ramp (without it the first action would sit `T/n` away on average — ~72 min for a 20/day campaign).
...
4. **`handle_check_messages`** (unified — merged old `handle_follow_up`) — `CHECK_MESSAGES_SLOTS_PER_DAY` (default 8) slots per campaign per day. Calls the Voyager conversations API to scan the inbox, filters conversations by matched `Lead.urn`, and handles both: (a) **lead reply**: syncs and replies via `run_follow_up_agent()`, resets `unanswered_follow_up_count`, stamps `deal.next_follow_up_at` for next nudge; (b) **nudge due**: sends proactive follow-up via `_handle_follow_up_nudge` when `deal.next_follow_up_at` has expired. Also catches CONNECTED deals not in the inbox (orphans, zero messages) for first outreach. Inbound inquiries from non-tracked users are skipped. The separate `plan_follow_up_window`, `enqueue_follow_up`, and `handle_follow_up` are removed. Lingering `FOLLOW_UP` tasks route to `handle_check_messages`. `BACKUP_follow_up.py` at project root holds the original implementation.
...
- **SiteConfig** (`linkedin/models.py`) — Singleton (pk=1). `llm_provider` (TextChoices: openai/anthropic/google/groq/mistral/cohere/openai_compatible), `llm_api_key`, `ai_model` (default `gpt-5-nano`), `conversation_ai_model` (default `gpt-5-mini`), `llm_api_base`. Accessed via `SiteConfig.load()`; `linkedin/llm.py:get_llm_model()` is the primary factory; `get_conversation_llm_model()` uses `conversation_ai_model` for the follow-up agent.
...
- **`llm.py`** — Two model factories + `run_agent_sync(coro)` sync boundary. `get_llm_model()` reads `ai_model` from `SiteConfig` and dispatches via per-provider builders (OpenAI / Anthropic / Google / Groq / Mistral / Cohere / openai_compatible) to the right `pydantic_ai.models.Model` — used by all system LLM calls (qualifier, keyword generation, summaries, etc.). `get_conversation_llm_model()` reads `conversation_ai_model` — used exclusively by the follow-up agent so a cheaper model can be assigned to chat interactions. Both builder functions accept a `model_field` kwarg to select which field to read. Call sites build `Agent(get_llm_model(), ...)` and invoke `run_agent_sync(agent.run(prompt))` — never `Agent.run_sync`, whose anyio portal leaves the caller's thread running-loop slot populated and poisons subsequent sync Playwright calls (`"using Playwright Sync API inside the asyncio loop"`). `run_agent_sync` drives the coroutine to completion on a short-lived worker thread with its own event loop; per-thread asyncio slots are independent, so the caller's thread stays clean regardless of what anyio / pytest-anyio / Jupyter / etc. did to it.
- **`exceptions.py`** — `AuthenticationError`, `TerminalStateError`, `SkipProfile`, `ReachedConnectionLimit`.
- **`onboarding.py`** — Interactive setup.
- **`agents/follow_up.py`** — Follow-up agent. Single LLM call with structured output (`FollowUpDecision`). Conversation is read in Python and injected into the prompt. No tool-calling loop. `decision.message` is sanitized post-LLM via `_strip_dashes()` — em/en dashes (`—`/`–`) the model emits despite the prompt rule are collapsed to a single space before the message is returned.
- **`agents/opportunity_detector.py`** — Opportunity detector. Runs on each new lead reply in the `check_messages` lead-reply path, BEFORE the follow-up agent responds. Single LLM call with structured output (`OpportunityAssessment`). If `notify_recommended` is true OR `opportunity_score >= SiteConfig.opportunity_score_threshold` (default 0.8), it sends a Telegram notification via `notify_opportunity()` and sets `Deal.opportunity_flagged = True` — a human-takeover flag that pauses all agent auto-replies (both the lead-reply path and `_handle_follow_up_nudge` skip when set). Assessment persisted on `Deal.opportunity_assessment` (JSONField). Testable offline via `python manage.py simulate_opportunity`.
...
- **`management/commands/industry_stats.py`** — `python manage.py industry_stats [--campaign NAME]`. Prints a table of Outreach rate by industry: Industry, Accepted%, Replied%, and Meeting count. Aggregates over Deals → Leads → ChatMessages via ContentType subquery.
...
- **`SiteConfig`** (DB singleton) — `llm_provider` (required, defaults to `openai`; choices: `openai`/`anthropic`/`google`/`groq`/`mistral`/`cohere`/`openai_compatible`), `llm_api_key` (required), `ai_model` (required, default `gpt-5-nano`), `conversation_ai_model` (required for follow-up agent, default `gpt-5-mini`), `llm_api_base` (required only for `openai_compatible`). Editable via Django Admin.
